...
const summaryMovie = (movie, list, origin) => {
    ...
    log(`Data de Lançamento: ${chalk.bold(new Date(movie.released).toLocaleDateString())}`);
    log(`Classificação Indicativa: ${chalk.bold(movie.certification)}`);
    log(`Duração do Filme: ${chalk.bold(humanizeDuration(movie.runtime * 60000, { language: "pt" }))}`);
...
const summaryByListPosition = (list, origin, position) => {
    const movie = origin == "popularMovies" ? list[position - 1] : list[position - 1].movie;
